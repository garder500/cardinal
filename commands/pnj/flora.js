...
class flora extends Command {
   ...
   const urimg = "https://tse2.mm.bing.net/th?id=OIP.ToFse6YPtLcjnlrEM0mo4gHaEK&pid=Api&P=0&w=281&h=159"
    const pseudo = "Flora (Princesse de l'abisse)"
...
fetch(`https://minria.fr/api/chatbot?message="${encodeURIComponent(m.content)}"&name=Flora&gender=femmale&pseudo=${encodeURIComponent(m.author.username)}`).then(res => res.json()).then(json => {
    console.log(json)
