...
class noelle extends Command {
   ...
   const urimg = "https://tse2.mm.bing.net/th?id=OIP.v1AUU_YkSy7qFLJ8Ho2glAHaKe&pid=Api&P=0&w=500&h=500"
    const pseudo = "Noelle (Princesse de l'abisse)"
...
message.channel.stopTyping(true);
       ...
       if(talkedRecently.has(message.author.id)) return;
                  function suite(){
const regex = RegExp("[Nn]o[eê]ll?e");
...
fetch(`https://minria.fr/api/chatbot?message="${encodeURIComponent(m.content)}"&name="Noelle"&gender="femmale"&pseudo=${encodeURIComponent(m.author.username)}`).then(res =>res.json()).then(json => {
    console.log(json)
